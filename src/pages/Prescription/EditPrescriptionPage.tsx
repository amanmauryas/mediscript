...
import { getPrescriptionById, updatePrescription } from '../../services/firebase';
...
const EditPrescriptionPage: React.FC = () => {
      ...
      try {
        ...
        if (prescription) {
          methods.reset({
            ...
            medicalHistory: prescription.medicalHistory?.join('\n') || '',
            allergies: prescription.allergies?.join('\n') || '',
            visitDate: prescription.visitDate.toDate().toISOString().split('T')[0],
            symptoms: prescription.symptoms.join('\n'),
            diagnosis: prescription.diagnosis.join('\n'),
            medications: prescription.medications,
            nonPharmacologicalAdvice: prescription.nonPharmacologicalAdvice?.join('\n') || '',
            labTests: prescription.labTests?.join('\n') || '',
            followUpDate: prescription.followUpDate?.toDate().toISOString().split('T')[0] || '',
            notes: prescription.notes || '',
            doctorInfo: {
              name: prescription.doctorInfo.name,
              specialization: prescription.doctorInfo.specialization,
              licenseNumber: prescription.doctorInfo.licenseNumber
            }
          });
        }
      } catch (error) {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      alert('An error occurred while updating the prescription. Please try again.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <MainLayout>
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-gray-900">Edit Prescription</h1>
